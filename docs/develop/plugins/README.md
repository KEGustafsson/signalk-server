...
A plugin runs inside the server process. The server keeps running when a plugin throws an uncaught exception or leaves a promise rejection unhandled: the error is logged and shown as the plugin's status in the Admin UI.
