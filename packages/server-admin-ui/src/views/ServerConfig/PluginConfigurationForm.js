...
const ArrayFieldItemTemplate = (props) => {
  ...
  return (
    <div className={CSS_CLASSES.ARRAY_ITEM}>
      ...
      <div className={`${GRID_COLUMNS.TOOLBAR} ${CSS_CLASSES.ARRAY_ITEM_TOOLBOX}`}>
        {hasToolbar && (
          <div
            className="btn-group"
...
const ObjectFieldTemplate = (props) => {
  ...
  return (
    <fieldset id={idSchema.$id}>
      ...
      {description && (
        <p id={`${idSchema.$id}__description`} className={CSS_CLASSES.FIELD_DESCRIPTION}>
          {description}
        </p>
      )}
...
export default ({ plugin, onSubmit }) => {
  ...
  const uiSchema = typeof plugin.uiSchema !== 'undefined'
