...
describe('Security', () => {
  ...
  it('Device delta includes labelName in source', async function () {
    this.timeout(10000)
    const deviceClientId = 'test-device-labelname'
    const deviceDescription = 'Test Device for LabelName'
    ...
    result = await fetch(
      `${url}/skServer/security/access/requests/${deviceClientId}/approved`,
      {
        method: 'PUT',
        headers: {
          'Content-Type': 'application/json',
    ...
    result = await fetch(
      `${url}/signalk/v1/access/requests/${deviceClientId}/response`
    )
    const responseJson = await result.json()
    const deviceToken = responseJson.accessRequest.token

    // 4. Connect a subscriber to receive deltas
    const subscriberPromiser = new WsPromiser(
      `ws://0.0.0.0:${port}/signalk/v1/stream?subscribe=all&token=${adminToken}`,
      2000
    )
    await subscriberPromiser.nextMsg() // hello message

    // 5. Connect device and send a delta
    const devicePromiser = new WsPromiser(
      `ws://0.0.0.0:${port}/signalk/v1/stream?subscribe=none&token=${deviceToken}`,
      2000
    )
    await devicePromiser.nextMsg() // hello message
    ...
    await devicePromiser.send(testDelta)

    // 6. Wait and check received delta has labelName
    // Allow some time for delta to propagate
    await new Promise((resolve) => setTimeout(resolve, 500))

    const messages = subscriberPromiser.parsedMessages()
    const deltaWithLabelName = messages.find(
      (msg) =>
        msg.updates &&
        msg.updates.some(
          (update) =>
            update.source &&
            update.source.labelName === deviceDescription
        )
    )

    if (!deltaWithLabelName) {
    ...
    deltaWithLabelName.should.not.be.undefined
    const update = deltaWithLabelName.updates.find(
      (u) => u.source && u.source.labelName
    )
    update.source.labelName.should.equal(deviceDescription)
    update.source.label.should.include('ws.')
  })
})
